...
otidalrouteUIDialogBase::otidalrouteUIDialogBase(wxWindow* parent,
                                                 wxWindowID id,
  ...
  sbSizer4 = new wxStaticBoxSizer(
      new wxStaticBox(this, wxID_ANY, wxT("Calculation")), wxHORIZONTAL);

  wxStaticBoxSizer* sbSizer3;
  sbSizer3 = new wxStaticBoxSizer(
      new wxStaticBox(sbSizer4->GetStaticBox(), wxID_ANY, wxT("Speed")),
      wxVERTICAL);

  m_tSpeed = new wxTextCtrl(sbSizer3->GetStaticBox(), wxID_ANY, wxEmptyString,
                            wxDefaultPosition, wxDefaultSize, 0);
  sbSizer3->Add(m_tSpeed, 0, wxALL, 5);

  wxStaticBoxSizer* sbSizer2;
  sbSizer2 =
      new wxStaticBoxSizer(new wxStaticBox(sbSizer3->GetStaticBox(), wxID_ANY,
                                           wxT("Start Date/Time")),
                           wxVERTICAL);

  m_textCtrl1 =
      new wxTextCtrl(sbSizer2->GetStaticBox(), wxID_ANY, wxEmptyString,
                     wxDefaultPosition, wxDefaultSize, 0);
  sbSizer2->Add(m_textCtrl1, 0, wxALL, 5);

  wxStaticBoxSizer* sbSizer91;
  sbSizer91 = new wxStaticBoxSizer(
      new wxStaticBox(sbSizer2->GetStaticBox(), wxID_ANY, wxT("DR/ETA")),
      wxVERTICAL);

  m_staticText4 =
      new wxStaticText(sbSizer91->GetStaticBox(), wxID_ANY, wxT("Route Name"),
                       wxDefaultPosition, wxDefaultSize, 0);
  m_staticText4->Wrap(-1);
  sbSizer91->Add(m_staticText4, 0, wxALL, 5);

  m_tRouteName =
      new wxTextCtrl(sbSizer91->GetStaticBox(), wxID_ANY, wxEmptyString,
                     wxDefaultPosition, wxDefaultSize, 0);
  sbSizer91->Add(m_tRouteName, 0, wxALL, 5);

  m_cbGPX = new wxCheckBox(sbSizer91->GetStaticBox(), wxID_ANY,
                           wxT("Save as GPX file"), wxDefaultPosition,
                           wxDefaultSize, 0);
  ...
  wxString m_choiceDepartureTimesChoices[] = {wxT("1"), wxT("2"), wxT("3"),
                                              wxT("4"), wxT("5"), wxT("6")};
  int m_choiceDepartureTimesNChoices =
      sizeof(m_choiceDepartureTimesChoices) / sizeof(wxString);
  m_choiceDepartureTimes = new wxChoice(
      sbSizer91->GetStaticBox(), wxID_ANY, wxDefaultPosition, wxDefaultSize,
      m_choiceDepartureTimesNChoices, m_choiceDepartureTimesChoices, 0);
  m_choiceDepartureTimes->SetSelection(0);
  sbSizer91->Add(m_choiceDepartureTimes, 0, wxALL, 5);

  m_bCalcDR =
      new wxButton(sbSizer91->GetStaticBox(), wxID_ANY, wxT("Calculate DR"),
                   wxDefaultPosition, wxDefaultSize, 0);
  sbSizer91->Add(m_bCalcDR, 0, wxALL | wxEXPAND, 5);

  m_bCalcETA =
      new wxButton(sbSizer91->GetStaticBox(), wxID_ANY, wxT("Calculate ETA"),
                   wxDefaultPosition, wxDefaultSize, 0);
  ...
  m_mSummary = new wxMenuItem(m_menu3, wxID_ANY, wxString(wxT("Summary")),
                              wxEmptyString, wxITEM_NORMAL);
  m_menu3->Append(m_mSummary);

  wxMenuItem* m_mNewRoute;
  m_mNewRoute =
      new wxMenuItem(m_menu3, wxID_ANY, wxString(wxT("View Route Tables")),
                     wxEmptyString, wxITEM_NORMAL);
  m_menu3->Append(m_mNewRoute);

  wxMenuItem* m_mDeleteAllRoutes;
  m_mDeleteAllRoutes =
      new wxMenuItem(m_menu3, wxID_ANY, wxString(wxT("Delete All Routes")),
                     wxEmptyString, wxITEM_NORMAL);
  ...
  m_mInformation = new wxMenuItem(m_mHelp, wxID_ANY,
                                  wxString(_("&Guide")) + wxT('\t') + wxT("F1"),
                                  wxEmptyString, wxITEM_NORMAL);
  ...
  this->Connect(wxEVT_CLOSE_WINDOW,
                wxCloseEventHandler(otidalrouteUIDialogBase::OnClose));
  this->Connect(wxEVT_SIZE,
                wxSizeEventHandler(otidalrouteUIDialogBase::OnSize));

  m_bCalcDR->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED,
      wxCommandEventHandler(otidalrouteUIDialogBase::DRCalculate), NULL, this);
  m_bCalcETA->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED,
      wxCommandEventHandler(otidalrouteUIDialogBase::ETACalculate), NULL, this);

  this->Connect(m_mSummary->GetId(), wxEVT_COMMAND_MENU_SELECTED,
                wxCommandEventHandler(otidalrouteUIDialogBase::OnSummary));
  this->Connect(m_mNewRoute->GetId(), wxEVT_COMMAND_MENU_SELECTED,
                wxCommandEventHandler(otidalrouteUIDialogBase::OnShowTables));
  this->Connect(
      m_mDeleteAllRoutes->GetId(), wxEVT_COMMAND_MENU_SELECTED,
      wxCommandEventHandler(otidalrouteUIDialogBase::OnDeleteAllRoutes));
  this->Connect(m_mInformation->GetId(), wxEVT_COMMAND_MENU_SELECTED,
                wxCommandEventHandler(otidalrouteUIDialogBase::OnInformation));
  this->Connect(m_mAbout->GetId(), wxEVT_COMMAND_MENU_SELECTED,
                wxCommandEventHandler(otidalrouteUIDialogBase::OnAbout));
}
...
otidalrouteUIDialogBase::~otidalrouteUIDialogBase() {
  ...
  m_bCalcDR->Disconnect(
      wxEVT_COMMAND_BUTTON_CLICKED,
      wxCommandEventHandler(otidalrouteUIDialogBase::DRCalculate), NULL, this);
  m_bCalcETA->Disconnect(
      wxEVT_COMMAND_BUTTON_CLICKED,
      wxCommandEventHandler(otidalrouteUIDialogBase::ETACalculate), NULL, this);
...
void ConfigurationDialog::OnDelete(wxCommandEvent& event) {
  ...
  for (std::list<TidalRoute>::iterator it =
           pPlugIn->m_potidalrouteDialog->m_TidalRoutes.begin();
       it != pPlugIn->m_potidalrouteDialog->m_TidalRoutes.end(); it++) {
    if ((*it).Name == rn) {
      pPlugIn->m_potidalrouteDialog->m_TidalRoutes.erase(it);
      m_lRoutes->Delete(s);

      pPlugIn->m_potidalrouteDialog->SaveXML(
...
AboutDialogBase::AboutDialogBase(wxWindow* parent, wxWindowID id,
  ...
  m_staticText135 =
      new wxStaticText(this, wxID_ANY, _("oTidalRoute Plugin Version"),
                       wxDefaultPosition, wxDefaultSize, 0);
